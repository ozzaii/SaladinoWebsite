...
// src/components/server/remote-picture.tsx
var remote_picture_exports = {};
__export(remote_picture_exports, {
  default: () => remote_picture_default
});
module.exports = __toCommonJS(remote_picture_exports);
...
// src/components/server/remote-picture.tsx
var import_picture = __toESM(require("../client/picture"));
var config = getConfig_default();
var RemotePicture = (0, import_react.forwardRef)(({ src, ...props }, forwardedRef) => {
  if (process.env.NODE_ENV === "production") {
    const nextImageConfig = process.env.__NEXT_IMAGE_OPTS;
    const allSizes = [...nextImageConfig.imageSizes, ...nextImageConfig.deviceSizes];
    for (const width of allSizes) {
      const outputInfo = buildOutputInfo_default({
        src,
        width,
        config
      });
      for (const { output, extension, originalExtension } of outputInfo) {
        ...
        const json = {
          ...
          src: `/${config.mode === "build" ? externalOutputDir.replace(/^_next/, ".next") : externalOutputDir}/${(0, import_node_crypto.createHash)(
            ...
            src.replace(/^https?:\/\//, "").split("/").slice(1).join("/")
          ).digest("hex")}.${originalExtension}`,
          width,
          extension,
          externalUrl: src
        };
        (0, import_fs_extra.appendFileSync)(
          (0, import_node_path.join)(process.cwd(), ".next/next-export-optimize-images-list.nd.json"),
          `${JSON.stringify(json)}
`
        );
      }
  ...
  return /* @__PURE__ */ import_react.default.createElement(import_picture.default, { ...props, src, ref: forwardedRef });
});
RemotePicture.displayName = "RemotePicture";
var remote_picture_default = RemotePicture;
//# sourceMappingURL=remote-picture.js.map